...
def _flat_veg_step(fields, n, **kwargs):
    ...
    compute_flow_fractions(fields.z, fields.mask, fields.flow_frac, kwargs.get("dx", 1.0), 1.0)
    fields.Q_daily.from_numpy(np.zeros((n, n), dtype=np.float32))
    vegetation_step(
        fields.V, fields.V_new, fields.M, fields.Q_daily,
        fields.flow_frac, fields.mask, **kwargs,
    )
...
def test_vegetation_growth_mortality_exact(grid):
    ...
    _flat_veg_step(
        fields, n,
        c=10.0, g_max=0.05, k1=5.0, d=0.13, Dp=0.0, dx=1.0, c1=0.0, c2=1.0, dt=1.0,
    )
...
def test_vegetation_steady_state(grid):
    ...
    # At V equilibrium, growth = mortality so V is stable at any level
    # Set V = 10 for testing
    V_eq = 10.0
    ...
    for _ in range(2000):
        ...
        vegetation_step(
            ...
            c=c, g_max=g_max, k1=k1, d=d, Dp=0.0, dx=1.0, c1=0.0, c2=1.0, dt=1.0,
        )
        fields.swap("V")
    ...
    assert dV < 0.01 * V_before, (
        f"|dV|={dV:.6f} > 1% of V={V_before:.4f}"
    )
...
def test_vegetation_isotropic_diffusion_exact(grid):
    ...
    _flat_veg_step(
        fields, n,
        c=0.0, g_max=0.0, k1=5.0, d=0.0, Dp=1.0, dx=1.0, c1=0.0, c2=1.0, dt=0.1,
    )
...
def test_vegetation_isotropic_diffusion_conserves(grid):
    ...
    for _ in range(100):
        vegetation_step(
            fields.V, fields.V_new, fields.M, fields.Q_daily,
            fields.flow_frac, fields.mask,
            c=0.0, g_max=0.0, k1=5.0, d=0.0, Dp=0.5, dx=1.0, c1=0.0, c2=1.0, dt=0.05,
        )
        fields.swap("V")
...
def test_vegetation_flow_dispersal_exact(grid):
    ...
    Upslope [1,2]: V=10, Q_daily=0.5, flow_frac[1,2,south]=1.0.
    ...
    assert frac[1, 2, 6] > 0.99, (
        f"Expected south frac ≈ 1.0, got {frac[1, 2, 6]:.4f}"
    )
    ...
    vegetation_step(
        fields.V, fields.V_new, fields.M, fields.Q_daily,
        fields.flow_frac, fields.mask,
        c=0.0, g_max=0.0, k1=5.0, d=0.0, Dp=0.0, dx=dx, c1=0.005, c2=0.0005, dt=1.0,
    )

    V_new = fields.V_new.to_numpy()

    # Hand computation
    q_neighbor = 0.5 / dx * 1000.0  # = 100
    seed_in = 1.0 * min(0.005 * q_neighbor * 10.0, 0.0005 * 10.0)  # min(5.0, 0.005) = 0.005
    q_self = 0.3 / dx * 1000.0  # = 60
...
def test_vegetation_flow_dispersal_conserves(grid):
    ...
    # Set Q_daily to something moderate
    Q_daily = np.zeros((n, n), dtype=np.float32)
    for i in range(1, n - 1):
        Q_daily[i, :] = float(i) * 0.01
    fields.Q_daily.from_numpy(Q_daily)
...
def test_vegetation_no_negative(grid):
    ...
    _flat_veg_step(
        fields, n,
        c=0.0, g_max=0.0, k1=5.0, d=10.0, Dp=0.0, dx=1.0, c1=0.0, c2=1.0, dt=1.0,
    )
